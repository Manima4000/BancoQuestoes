...
const assuntosPorMateria: Record<string, { nome: string; slug: string; descricao?: string; ordem: number }[]> = {
    matematica: [
...
export async function seedAssuntos() {
    ...
    for (const [materiaSlug, assuntos] of Object.entries(assuntosPorMateria)) {
        ...
        const materia = await Materia.findOne({ slug: materiaSlug });
        ...
        for (const assunto of assuntos) {
            const existe = await Assunto.findOne({
